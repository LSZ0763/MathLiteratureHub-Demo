...
export default function SearchResultsView({
  ...
  const [rightWidth, setRightWidth] = useState(55);
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (showDetail) {
        if (dragTarget === 'left') {
          const newLeft = Math.max(20, Math.min(50, pct));
          setLeftWidth(newLeft);
          setRightWidth(Math.max(20, 100 - newLeft - 30));
        } else if (dragTarget === 'right') {
          const newRight = Math.max(20, Math.min(50, 100 - pct));
          setRightWidth(newRight);
          setLeftWidth(Math.max(20, 100 - newRight - 30));
        }
      } else {
  ...
  }, [isDragging, dragTarget, showDetail]);
    ...
    <div className="flex flex-col h-full">
      ...
      >
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {pagePapers.map((paper) => {
              ...
              return (
                ...
                >
                  <h4 className="text-sm font-semibold text-gray-900" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{paper.title}</h4>
                  <p className="text-xs text-gray-600 mt-0.5" style={{ display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{authorsStr}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {paper.journal || paper.source} · {paper.published_date}
                  </p>
                  <p className="text-xs text-gray-500 mt-1" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.4' }}>
                    {paper.summary_raw || '（暂无摘要）'}
                  </p>
                  {keywords.length > 0 && (
                    <p className="text-xs text-indigo-600 mt-1" style={{ display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {keywords.join(', ')}
                    </p>
                  )}
            ...
            {pagePapers.length === 0 && (
              <div className="text-center text-gray-400 py-10">暂无文献</div>
            )}
          </div>
        ...
        <div className="flex flex-col overflow-hidden" style={{ width: `${rightWidth}%` }}>
          <div className="text-xs font-semibold text-gray-500 px-3 py-2 border-b border-gray-100 bg-gray-50">
            引用关系图
          </div>
        ...
        {/* Right: Detail Panel */}
        {showDetail && (
          <div className="flex flex-col overflow-hidden flex-1" style={{ minWidth: '200px' }}>
            <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100 bg-gray-50">
              <span className="text-xs font-semibold text-gray-500">文献详情</span>
              <button onClick={handleCloseDetail} className="text-xs text-gray-400 hover:text-gray-600">✕</button>
            </div>
